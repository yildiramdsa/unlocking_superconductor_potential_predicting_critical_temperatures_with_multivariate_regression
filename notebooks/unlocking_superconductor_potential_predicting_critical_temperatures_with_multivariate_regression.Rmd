...
final_int_model = lm(critical_temp ~ range_atomic_radius:wtd_mean_ThermalConductivity
                 ...
                 +wtd_mean_fie:wtd_mean_ElectronAffinity
                 +wtd_entropy_fie:wtd_entropy_Density
                 +wtd_entropy_fie:mean_ElectronAffinity, data=superconductivity, ,na.action=na.exclude)
...
```{r}
ggplot(data=final_reduced, aes(residuals(final_reduced))) +
...
```{r}
#normal QQ plot
ggplot(final_reduced, aes(sample=final_reduced$residuals)) +
stat_qq() +
stat_qq_line()
```
...
```{r}
set.seed(26)
sample_res <- sample(residuals(temp_model_linearity_ass), 5000)
...
```{r}
superconductivity[cooks.distance(final_reduced)> 0.5]
```
```{r}
plot(final_reduced,pch=18,col="red",which=c(4))
```

```{r}
lev=hatvalues(final_reduced)
p = length(coef(final_reduced))
n = nrow(final_reduced)
outlier3p = lev[lev>(3*p/n)]
print(outlier3p)
```

```{r}
plot(rownames(final_reduced),lev, main = "Leverage in superconductivity Dataset",xlab= "observation", ylab = "Leverage Value")
